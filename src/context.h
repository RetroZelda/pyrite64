...
struct Context
{
  ...
  // Editor state
  uint64_t openedCanvasUUID{0};  // UUID of the currently open canvas (0 = none)
  uint64_t selAssetUUID{0};
  // Relative folder under /assets the Prefabs tab is currently browsing ("" = root).
  // New prefabs ("To Prefab") are created here so they can be organized into subfolders.
  std::string prefabCreateDir{};
